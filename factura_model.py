...
class Factura(Base):
    ...
    @property
    def cuota_cubierta_hasta(self):
        """La última cuota que ya quedó completamente pagada (por número),
        o None si todavía no se ha pagado ninguna. Es la pieza que arma el
        aviso 'cubre hasta X'."""
        pagadas = sorted(
            (c for c in self.cuotas if c.estado == EstadoFactura.pagada),
            key=lambda c: c.numero_cuota,
        )
        return pagadas[-1] if pagadas else None

    @property
    def aviso_periodo_cubierto(self) -> Optional[str]:
        """Texto corto y listo para mostrar, ej. 'Cubre hasta 15 nov' — la
        ÚNICA definición de este aviso en todo el proyecto (mismo criterio
        que texto_cuota arriba). Explica, junto a fecha_vencimiento_vigente,
        por qué la próxima fecha de pago puede 'saltar' varios periodos de
        golpe cuando se pagan varias cuotas juntas: no es un error de
        cálculo, es que ya se adelantó ese tramo del préstamo.
        None cuando no hay nada pagado todavía (préstamo recién emitido) —
        en ese caso no hay nada que 'cubrir' y el aviso no aporta nada."""
        ultima_cubierta = self.cuota_cubierta_hasta
        if not ultima_cubierta:
            return None
        meses = ["ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"]
        f = ultima_cubierta.fecha_vencimiento
        return f"Cubre hasta {f.day} {meses[f.month - 1]}"
